"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.WebSocketProvider = void 0;
const websockets_1 = require("@nestjs/websockets");
const socket_io_1 = require("socket.io");
const timers_1 = require("timers");
let WebSocketProvider = class WebSocketProvider {
    onModuleInit() {
        this.server.on('connection', (client) => {
            console.log(`Client connected (ID: ${client.id}})`);
            client.emit('timer', this.timerValue);
        });
    }
    constructor() {
        this.timerValue = 0;
        this.startTimer();
    }
    startTimer() {
        const randomTime = Math.floor(Math.random() * 10000) + 5000;
        console.log(`Next timer will end in ${randomTime / 1000} seconds`);
        setTimeout(() => {
            (0, timers_1.clearInterval)(this.interval);
            (0, timers_1.clearInterval)(this.intervalSync);
            this.timerValue = 0;
            this.server.emit('timer', this.timerValue);
            this.startTimer();
        }, randomTime);
        this.interval = setInterval(() => {
            this.timerValue += 0.1;
        }, 100);
        this.intervalSync = setInterval(() => {
            this.server.emit('timer', this.timerValue);
        }, 1500);
    }
};
exports.WebSocketProvider = WebSocketProvider;
__decorate([
    (0, websockets_1.WebSocketServer)(),
    __metadata("design:type", socket_io_1.Server)
], WebSocketProvider.prototype, "server", void 0);
exports.WebSocketProvider = WebSocketProvider = __decorate([
    (0, websockets_1.WebSocketGateway)({ cors: true }),
    __metadata("design:paramtypes", [])
], WebSocketProvider);
//# sourceMappingURL=websocket.js.map